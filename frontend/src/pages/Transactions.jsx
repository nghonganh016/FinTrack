...
function AddTxModal({ onClose, onDone }) {
  ...
  const submit = async (e) => {
    ...
    if (type === 'Expense' && !catID) return setError('Select a category.')
    if (!accID) return setError('Select an account.')
    setLoading(true)
    try {
...
export default function Transactions() {
  ...
  const load = useCallback(async () => {
    setLoading(true)
    try {
      const r = await api.get('/transactions', { params: { search, type: filter } })
      setRows(r.data.rows)
  ...
  return (
    <div className="p-7 space-y-5">
      ...
      <Card className="flex flex-wrap items-center gap-3 px-4 py-3">
        <div className="flex-1 min-w-[200px] flex items-center gap-4 bg-gray-50 border border-gray-200 rounded-lg px-3 h-9">
            ...
            <table className="w-full text-sm">
              <thead>
                <tr className="border-b border-gray-100">
                  {['','Date','Description','Category','Status','Amount',''].map((h, i) => (
                ...
                {rows.map((row, i) => {
                  ...
                  return (
                    <tr key={i} className="hover:bg-gray-50/50 transition-colors">
                      ...
                      <td className="px-4 py-3 text-gray-800 max-w-[180px] truncate">{row.descr || '—'}</td>
